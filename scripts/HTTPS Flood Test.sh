...
# Функция для отправки HTTPS-запросов
send_https_requests() {
    local target=$1
    local port=$2
    local duration=$3
    ...
    while [ $(($(date +%s) - start_time)) -lt $duration ]; do
        if curl -s -k -o /dev/null -w "%{http_code}" https://$target:$port/ > /dev/null 2>&1; then
            requests=$((requests + 1))
        else
            errors=$((errors + 1))
        fi
...
for ((i=0; i<THREADS; i++)); do
    send_https_requests "$TARGET" "$PORT" "$DURATION" "$i" >> "$TEMP_FILE" &
    pids[$i]=$!
done
